...
export default class EnemyScene extends Phaser.Scene {
    ...
    preload() {
        ...
        // ─── Enemy Spritesheets ────────────────────────
        this.load.spritesheet('enemy-walk', 'assets/images/enemy/Walk.png', {
            frameWidth: 96, frameHeight: 96
        })
        this.load.spritesheet('enemy-attack', 'assets/images/enemy/Attack_1.png', {
            frameWidth: 96, frameHeight: 96
        })
        this.load.spritesheet('enemy-idle', 'assets/images/enemy/Idle.png', {
            frameWidth: 96, frameHeight: 96
        })
        this.load.spritesheet('enemy-hurt', 'assets/images/enemy/Hurt.png', {
            frameWidth: 96, frameHeight: 96
        })
        this.load.spritesheet('enemy-dead', 'assets/images/enemy/Death.png', {
            frameWidth: 96, frameHeight: 96
        })
        ...
        this.load.spritesheet('player-shield', 'assets/images/player/Shield.png', {
            frameWidth: 128, frameHeight: 128
        })
        this.load.spritesheet('player-jump', 'assets/images/player/Jump.png', {
            frameWidth: 128, frameHeight: 128
        })
        this.load.spritesheet('player-idle', 'assets/images/player/Idle.png', {
            frameWidth: 128, frameHeight: 128
        })
        this.load.spritesheet('player-hurt', 'assets/images/player/Hurt.png', {
            frameWidth: 128, frameHeight: 128
        })
        this.load.spritesheet('player-dead', 'assets/images/player/Dead.png', {
            frameWidth: 128, frameHeight: 128
        })
        this.load.spritesheet('player-attack', 'assets/images/player/Attack_1.png', {
            frameWidth: 128, frameHeight: 128
        })
        this.load.spritesheet('player-attack2', 'assets/images/player/Attack_2.png', {
            frameWidth: 128, frameHeight: 128
        })
        this.load.spritesheet('player-attack3', 'assets/images/player/Attack_3.png', {
            frameWidth: 128, frameHeight: 128
        })
    }
    ...
    create() {
        ...
        // ─── Player ────────────────────────────────────
        this.player = this.physics.add.sprite(100, H - 200, 'player-idle')
            .setDepth(10)
            .setScale(2)
        ...
        this.input.on('pointerdown', (pointer) => {
            ...
            // If collider tool is enabled, right click should draw, not attack
            if (pointer.rightButtonDown()) {
                if (!this.colliderTool?.enabled) {
                    this.useAbility3()
                }
            }
        ...
        // ─── Checkpoints ───────────────────────────────
        this.checkpoints = []
        this.lastCheckpoint = { x: 100, y: H - 200 }
        this.createCheckpoint(1000, H - 60)
        this.createCheckpoint(2200, H - 60)
        this.createCheckpoint(3400, H - 60)
        this.createCheckpoint(4600, H - 60)
        this.createCheckpoint(5800, H - 60)
        ...
        this.progressBarBg = this.add.rectangle(W / 2, 75, W - 200, 8, 0x222222)
            .setScrollFactor(0).setDepth(99)
        this.progressBar = this.add.rectangle(100, 75, 0, 6, 0x00ff88)
        ...
        // ─── Collision Placer Tool ─────────────────────
        // T to enable when you need it
        this.colliderTool = {
            rects: [],
            startX: 0,
            startY: 0,
        ...
        this.input.on('pointerdown', (pointer) => {
            ...
            if (!this.colliderTool.enabled) return

            this.colliderTool.startX = pointer.worldX
            this.colliderTool.startY = pointer.worldY
            this.colliderTool.drawing = true

            this.colliderTool.preview = this.add.rectangle(
                pointer.worldX, pointer.worldY, 1, 1, 0x00ff00, 0.3
            ).setStrokeStyle(2, 0x00ff00).setDepth(900)
        })

        this.input.on('pointermove', (pointer) => {
            if (!this.colliderTool.drawing) return
            const sx = this.colliderTool.startX
            const sy = this.colliderTool.startY
            const w = pointer.worldX - sx
            const h = pointer.worldY - sy
            if (this.colliderTool.preview) {
                this.colliderTool.preview.setPosition(sx + w / 2, sy + h / 2)
                this.colliderTool.preview.setSize(Math.abs(w), Math.abs(h))
            }
        })
        ...
        this.input.on('pointerup', (pointer) => {
            ...
            this.colliderTool.drawing = false

            const sx = this.colliderTool.startX
            const sy = this.colliderTool.startY
            const ex = pointer.worldX
            const ey = pointer.worldY
            const x = Math.round((sx + ex) / 2)
            const y = Math.round((sy + ey) / 2)
            const w = Math.round(Math.abs(ex - sx))
            const h = Math.round(Math.abs(ey - sy))

            if (this.colliderTool.preview) {
                this.colliderTool.preview.destroy()
                this.colliderTool.preview = null
            }

            if (w < 5 || h < 5) return

            const rect = this.add.rectangle(x, y, w, h, 0x00ff00, 0.2)
                .setStrokeStyle(2, 0x00ff00).setDepth(900)
            const label = this.add.text(x, y, `${x}, ${y}\n${w}x${h}`, {
                fontSize: '12px',
                fill: '#00ff00',
                align: 'center',
            ...
            this.colliderTool.rects.push({ x, y, w, h, rect, label })
            console.log('─── COLLIDERS ───')
            this.colliderTool.rects.forEach(r => {
                console.log(`this.createPlatform(${r.x}, ${r.y}, ${r.w}, ${r.h})`)
            })
        })

        this.input.keyboard.on('keydown-Z', () => {
            const last = this.colliderTool.rects.pop()
            if (last) {
                last.rect.destroy()
                last.label.destroy()
            }
        ...
        this.input.keyboard.on('keydown-T', () => {
            this.colliderTool.enabled = !this.colliderTool.enabled
            console.log(`Collider Tool: ${this.colliderTool.enabled ? 'ON' : 'OFF'}`)
            this.colliderTool.rects.forEach(r => {
                r.rect.setVisible(this.colliderTool.enabled)
                r.label.setVisible(this.colliderTool.enabled)
            })
        })
    }
    ...
    createAnimations() {
        ...
        if (!this.anims.exists('enemy-attack')) {
            this.anims.create({
                key: 'enemy-attack',
                frames: this.anims.generateFrameNumbers('enemy-attack', { start: 0, end: 5 }),
                frameRate: 10,
                repeat: 0
            })
        }
        ...
        if (!this.anims.exists('enemy-hurt')) {
            this.anims.create({
                key: 'enemy-hurt',
                frames: this.anims.generateFrameNumbers('enemy-hurt', { start: 0, end: 1 }),
                frameRate: 8,
                repeat: 0
            })
        }
        ...
        if (!this.anims.exists('enemy-dead')) {
            this.anims.create({
                key: 'enemy-dead',
                frames: this.anims.generateFrameNumbers('enemy-dead', { start: 0, end: 5 }),
                frameRate: 10,
                repeat: 0
            })
        }
        ...
        this.anims.create({
            key: 'player-shield',
            frames: this.anims.generateFrameNumbers('player-shield', { start: 0, end: 1 }),
            frameRate: 8,
            repeat: 0
        })
        ...
        this.anims.create({
            key: 'player-dead',
            frames: this.anims.generateFrameNumbers('player-dead', { start: 0, end: 2 }),
            frameRate: 6,
            repeat: 0
        })
        ...
        this.anims.create({
            key: 'player-attack2',
            frames: this.anims.generateFrameNumbers('player-attack2', { start: 0, end: 3 }),
            frameRate: 12,
            repeat: 0
        })
        ...
        this.anims.create({
            key: 'player-attack3',
            frames: this.anims.generateFrameNumbers('player-attack3', { start: 0, end: 2 }),
            frameRate: 12,
            repeat: 0
        })
    }
    ...
    update() {
        ...
        if (this.isOnLadder) {
            ...
            if (this.cursors.up.isDown || this.wasd.up.isDown) {
                this.player.body.setVelocityY(-climbSpeed)
        ...
        if (this.isAttacking || this.isMeleeAttacking || this.isUsingAbility) {
            this.updateEnemies()
            this.cleanupBullets()
            this.updateProgress()
            return
        ...
        if (Phaser.Input.Keyboard.JustDown(this.cKey) && this.canShield && !this.isDead) {
            this.useShield()
            return
        }

        if (Phaser.Input.Keyboard.JustDown(this.qKey) && this.canHighJump && this.player.body.onFloor()) {
            this.useHighJump()
        }

        if (Phaser.Input.Keyboard.JustDown(this.eKey) && this.canAbility2 && !this.isDead) {
            this.useAbility2()
            return
        }
    ...
    useShield() {
        ...
        this.shieldIndicator.setText('🛡️ ACTIVE...')
        this.shieldIndicator.setFill('#ffffff')

        this.player.once('animationcomplete-player-shield', () => {
            this.isUsingAbility = false
        })
        ...
        this.time.delayedCall(this.shieldDuration, () => {
            ...
            this.isInvincible = false
            this.isUsingAbility = false
            this.shieldIndicator.setText('🛡️ COOLDOWN')
            this.shieldIndicator.setFill('#666666')
        })

        this.time.delayedCall(this.shieldCooldown, () => {
            this.canShield = true
            this.shieldIndicator.setText('🛡️ SHIELD [C]')
            this.shieldIndicator.setFill('#4488ff')
        })
    }
    ...
    useHighJump() {
        ...
        this.highJumpIndicator.setText('🚀 COOLDOWN')
        this.highJumpIndicator.setFill('#666666')

        for (let i = 0; i < 8; i++) {
            const trail = this.add.circle(
                this.player.x + Phaser.Math.Between(-15, 15),
                this.player.y + 30 + (i * 10),
        ...
        this.time.delayedCall(this.highJumpCooldown, () => {
            this.canHighJump = true
            this.highJumpIndicator.setText('🚀 HIGH JUMP [Q]')
            this.highJumpIndicator.setFill('#ff44ff')
        })
    }

    // ═══════════════════════════════════════════════════
    // ─── ATTACK 2 (E) — Attack_2.png ───────────────────
    // ═══════════════════════════════════════════════════

    useAbility2() {
        if (!this.canAbility2 || this.isDead) return

        this.canAbility2 = false
        this.isUsingAbility = true

        this.ability2Indicator.setText('⚔️ COOLDOWN')
        this.ability2Indicator.setFill('#666666')
        ...
        this.time.delayedCall(150, () => {
            ...
            const hitbox = this.add.rectangle(hitX, hitY, 140, 90, 0xffaa00, 0)
                .setDepth(15)
            ...
            arc.lineStyle(4, 0xffaa00, 0.8)
            if (direction === 1) {
                arc.beginPath()
                arc.arc(this.player.x + 20, this.player.y - 10, 110,
                    Phaser.Math.DegToRad(-70), Phaser.Math.DegToRad(70), false)
                arc.strokePath()
            } else {
                arc.beginPath()
                arc.arc(this.player.x - 20, this.player.y - 10, 110,
                    Phaser.Math.DegToRad(110), Phaser.Math.DegToRad(250), false)
                arc.strokePath()
            }
            ...
            this.physics.add.overlap(hitbox, this.enemies, (hb, enemy) => {
                ...
                enemy.once('animationcomplete-enemy-hurt', () => {
                    if (enemy.active) {
                        enemy.play(enemy.enemyType === 'patrol' ? 'enemy-walk' : 'enemy-idle', true)
                    }
                })
        ...
        this.player.once('animationcomplete-player-attack2', () => {
            this.isUsingAbility = false
        })
        ...
        this.time.delayedCall(this.ability2Cooldown, () => {
            this.canAbility2 = true
            this.ability2Indicator.setText('⚔️ ATTACK 2 [E]')
            this.ability2Indicator.setFill('#ffaa00')
        })
    }

    // ═══════════════════════════════════════════════════
    // ─── ATTACK 3 (RMB) — Attack_3.png ─────────────────
    // ═══════════════════════════════════════════════════

    useAbility3() {
        if (!this.canAbility3 || this.isDead) return

        this.canAbility3 = false
        this.isUsingAbility = true

        this.ability3Indicator.setText('💥 COOLDOWN')
        this.ability3Indicator.setFill('#666666')
        ...
        this.time.delayedCall(100, () => {
            ...
            const hitbox = this.add.rectangle(hitX, hitY, 160, 100, 0xff4444, 0)
                .setDepth(15)
            this.physics.add.existing(hitbox)
            hitbox.body.setAllowGravity(false)
            ...
            // ─── Slash visual ──────────────────────────
            const arc = this.add.graphics().setDepth(12)
            arc.lineStyle(5, 0xff4444, 0.9)
            if (direction === 1) {
                arc.beginPath()
                arc.arc(this.player.x + 30, this.player.y - 10, 130,
                    Phaser.Math.DegToRad(-80), Phaser.Math.DegToRad(80), false)
                arc.strokePath()
            } else {
                arc.beginPath()
                arc.arc(this.player.x - 30, this.player.y - 10, 130,
                    Phaser.Math.DegToRad(100), Phaser.Math.DegToRad(260), false)
                arc.strokePath()
            }
            ...
            for (let i = 0; i < 8; i++) {
                const spark = this.add.rectangle(
                    hitX + Phaser.Math.Between(-50, 50),
                    hitY + Phaser.Math.Between(-40, 40),
                ...
                this.tweens.add({
                    targets: spark,
                    x: spark.x + (direction * Phaser.Math.Between(30, 80)),
            ...
            this.physics.add.overlap(hitbox, this.enemies, (hb, enemy) => {
                ...
                enemy.body.setVelocityX(direction * 500)
        ...
        this.player.once('animationcomplete-player-attack3', () => {
            this.isUsingAbility = false
        })
        ...
        this.time.delayedCall(this.ability3Cooldown, () => {
            this.canAbility3 = true
            this.ability3Indicator.setText('💥 ATTACK 3 [RMB]')
            this.ability3Indicator.setFill('#ff4444')
        })
    }

    // ═══════════════════════════════════════════════════
    // ─── MELEE ATTACK (LMB) — Attack_1.png ─────────────
    // ═══════════════════════════════════════════════════
    ...
    meleeAttack() {
        ...
        this.meleeIndicator.setText('🗡️ COOLDOWN')
        this.meleeIndicator.setFill('#666666')
        ...
        this.time.delayedCall(100, () => {
            ...
            this.katanaHitbox = this.add.rectangle(
                slashX, slashY, this.meleeRange, 80, 0xff8800, 0
            ).setDepth(15)

            this.physics.add.existing(this.katanaHitbox)
            this.katanaHitbox.body.setAllowGravity(false)

            const slashArc = this.add.graphics().setDepth(12)
            slashArc.lineStyle(3, 0xffffff, 0.9)

            if (direction === 1) {
                slashArc.beginPath()
                slashArc.arc(
                    this.player.x + 20, this.player.y - 10,
                    this.meleeRange - 20,
                    Phaser.Math.DegToRad(-60), Phaser.Math.DegToRad(60), false
                )
                slashArc.strokePath()
            } else {
                slashArc.beginPath()
                slashArc.arc(
                    this.player.x - 20, this.player.y - 10,
                    this.meleeRange - 20,
                    Phaser.Math.DegToRad(120), Phaser.Math.DegToRad(240), false
                )
                slashArc.strokePath()
            }
            ...
            for (let i = 0; i < 5; i++) {
                ...
                this.tweens.add({
                    targets: spark,
                    x: sparkX + (direction * Phaser.Math.Between(20, 60)),
                    y: sparkY + Phaser.Math.Between(-20, 20),
                    alpha: 0,
                    duration: 200,
                    onComplete: () => spark.destroy()
                })
            ...
            this.time.delayedCall(120, () => {
                if (this.katanaHitbox) {
                    this.katanaHitbox.destroy()
                    this.katanaHitbox = null
                }
                slashArc.destroy()
            })
        ...
        this.time.delayedCall(this.meleeCooldown, () => {
            this.canMelee = true
            this.meleeIndicator.setText('🗡️ ATTACK 1 [LMB]')
            this.meleeIndicator.setFill('#ff8800')
        })
    }
    ...
    katanaHitEnemy(hitbox, enemy) {
        ...
        enemy.body.setVelocityX(knockDir * 400)
    ...
    dash() {
        ...
        const trail = this.add.rectangle(
            this.player.x, this.player.y, 64, 96, 0x00ffff, 0.3
        ...
        this.time.delayedCall(this.dashCooldown, () => {
            this.canDash = true
            this.dashIndicator.setText('⚡ DASH [SHIFT]')
    ...
    playerDie() {
        this.isDead = true
        this.player.play('player-dead', true)
        ...
        const deathText = this.add.text(W / 2, H / 2 - 30, '💀 DEFEATED', {
            fontSize: '48px',
            fill: '#ff0000',
            fontStyle: 'bold'
        }).setOrigin(0.5).setScrollFactor(0).setDepth(999)
        const respawnText = this.add.text(W / 2, H / 2 + 30, 'Respawning...', {
            fontSize: '20px',
            fill: '#888888'
        }).setOrigin(0.5).setScrollFactor(0).setDepth(999)
    ...
    updateHealthBar() {
        ...
        if (ratio > 0.6) this.healthBar.setFillStyle(0x00ff88)
        else if (ratio > 0.3) this.healthBar.setFillStyle(0xffaa00)
    ...
    createEnemy(x, y, type, patrolRange) {
        const enemy = this.physics.add.sprite(x, y, 'enemy-idle')
            .setScale(2.66)
            .setDepth(5)

        // collider tuned for scale 2.66
        enemy.body.setSize(32, 44)
        enemy.body.setOffset(32, 48)
        ...
        this.enemies.getChildren().forEach(enemy => {
            ...
            if (enemy.enemyType === 'patrol') {
                ...
                if (dist < 100 && !enemy.isAttacking) {
                    ...
                    enemy.once('animationcomplete-enemy-attack', () => {
                        enemy.isAttacking = false
                    })
                    this.time.delayedCall(600, () => {
                        enemy.isAttacking = false
    ...
    enemyDeath(enemy) {
        ...
        const scoreText = this.add.text(enemy.x, enemy.y - 20, '+50', {
            fontSize: '18px',
            fill: '#ffff00',
            fontStyle: 'bold'
        ...
        enemy.once('animationcomplete-enemy-dead', () => {
            enemy.destroy()
        })
    ...
    createSpikes(x, y, width, height) {
        ...
        this.add.text(x, y - 25, '⚠️', { fontSize: '14px' })
            .setOrigin(0.5)
            .setDepth(2)
    ...
    createCheckpoint(x, y) {
        ...
        const flag = this.add.text(x, y - 40, '🏁', { fontSize: '24px' })
            .setOrigin(0.5)
            .setDepth(4)
        ...
        this.physics.add.overlap(this.player, cp, () => {
            if (this.lastCheckpoint.x !== x) {
                ...
                const saved = this.add.text(x, y - 70, '✅ Checkpoint! HP restored', {
                    fontSize: '14px',
                    fill: '#00ff88'
                }).setOrigin(0.5).setDepth(20)
    ...
    cleanupBullets() {
        ...
        this.bullets.getChildren().forEach(bullet => {
            if (
                bullet.x < bounds.x - 50 || bullet.x > bounds.right + 50 ||
